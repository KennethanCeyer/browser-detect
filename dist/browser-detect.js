...
function browserDetector(userAgent) {
    if (!userAgent)
        userAgent = !navigatorImpl ? '' : (navigatorImpl.userAgent || navigatorImpl.vendor);
    return runDetect(userAgent);
}
...
function runDetect(userAgent) {
    ...
    var result = __assign({}, checkBrowser(userAgent), checkMobile(userAgent), checkOs(userAgent));
    return result;
}
